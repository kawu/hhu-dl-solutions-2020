from typing import Tuple, List, TypedDict, NewType, Set
...
def words_in(data: List[Tuple[Inp, Out]]) -> Set[Word]:
    """Determine the set of words in a given dataset"""
    words = set()
    for sent, _ in data:
        for word in sent:
            words.add(word)
...
def collect_OOVs(train_data: List[Tuple[Inp, Out]], dev_data: List[Tuple[Inp, Out]]):
    ...
    ['chased', 'dog']
    """
    return words_in(dev_data) - words_in(train_data)
